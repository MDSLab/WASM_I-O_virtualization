...
#include "service_app_wasm.h"
//#include "led_toggle_app_wasm.h"
...
/* ── Thread config ───────────────────────────────────────────────────────── */
#define WASM_THREAD_STACK_SIZE   3072
...
static void wasm_thread_entry(void *p1, void *p2, void *p3)
{
    ...
    if (!wasm_runtime_call_wasm(exec_env, func, 0, NULL)) {
        printk("Exec error: %s",wasm_runtime_get_exception(ctx->instance));
        ctx->result = -EFAULT;
    }
...
void main(void)
{
    ...
    wasm_module_t module = wasm_runtime_load(
        (uint8_t*) service_app_aot, service_app_aot_len,
    ...
    printk("WASM instance created, spawning thread\n");
